...
class JaxGemmaEngine:
    ...
    def __init__(
        ...
        # Prefill, not decode, sets the batch ceiling. Measured ON E2B, v6e-1, by
        ...
        #
        # DO NOT generalize that to wider models — it is false on the 31B, which has
        # 32 attention heads to E2B's 8. Same measurement there (2026-07-31): temp is
        # 9.17 / 10.19 / 10.34 GB at S = 1K / 2K / 4K, then 38.28 GB at 8K. XLA stops
        # tiling somewhere between 4K and 8K and the ceiling arrives as a cliff, not
        # a slope, so no per-token budget describes it. On a v6e-1 the 31B admits
        # ~4096 prompt tokens per pass and nothing anywhere near 11,900.
        # See benchmarks/runs/2026-07-31-gemma4-31b-v6e1/REPORT.md.
        #
