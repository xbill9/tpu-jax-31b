...
_KEQV_POSTNORM = os.environ.get("JAX_E_KEQV_POSTNORM", "0") == "1"
_KEQV_POSTROPE = os.environ.get("JAX_E_KEQV_POSTROPE", "0") == "1"
...
class Gemma4EAttentionJAX:
    ...
    def __init__(self, config: Gemma4EConfig, layer_idx: int):
        ...
        # attention_k_eq_v layers ship no v_proj; the loader aliases V's weights to
        # K's, so V is the raw K projection with neither k_norm nor RoPE applied.
        # JAX_E_KEQV_POSTNORM=1 selects the alternative reading, "V is K after
        # k_norm". TESTED AND REJECTED on gemma-4-12B (2026-07-31): it degrades
        # output further (top-1 becomes '<image|>' on "The capital of France is",
        # vs '1' for the default). Kept as a gated ablation so the next person does
        # not re-run it blind. Both readings are wrong — the 12B defect is
        # elsewhere; see docs/12b-exploration-2026-07-31.md.
        # Static Python bools only; nothing here may enter the params pytree.
        self.k_eq_v = bool(config.attention_k_eq_v)
    ...
    def __call__(
        ...
        if self.is_shared:
            ...
            if _KEQV_POSTNORM and self.k_eq_v and params.get("v_norm") is None:
                # Experimental reading: V is K *after* k_norm (still pre-RoPE).
                v = k
        ...
        if not self.is_shared:
            ...
            if _KEQV_POSTROPE and self.k_eq_v and params.get("v_norm") is None:
                # Third reading of "V is K": V is the post-RoPE key.
                v = k
...
class Gemma4EModelJAX:
    ...
    def __call__(
        ...
        """logits_at: optional [B] int32 positions. When given, the lm_head is
        applied ONLY at those positions and the returned logits are [B, 1, V]
        instead of [B, S, V].

        Mathematically exact, but NOT bitwise on TPU. Matmul rows are
        independent and the softcap is elementwise, so in exact arithmetic the
        selected row is the same either way — and on CPU it is bitwise identical
        (tests/test_prefill_logits_slice.py). On a TPU running the real 31B at
        W4A16, it is not: contracting [B,1,H]x[H,V] tiles and accumulates
        differently from [B,S,H]x[H,V], and measured 2026-07-31 the two paths
        differ in ~every logit with max |diff| ~3.7e-2 (argmax unchanged at the
        shapes tested). Do not describe this as bitwise; an earlier version of
        this comment did, on the strength of the CPU test alone.

        It exists because prefill needs exactly one row per sequence and
        materializing all S of them costs 0.52 MB/token at bf16 and 1.05 at f32
        (vocab 262144). On the 31B at S=8192 that is 4.3-8.6 GB of pure waste and
        it is what sets the one-shot prefill ceiling on a v6e-1 — see
        benchmarks/runs/2026-07-31-gemma4-31b-v6e1/REPORT.md.
        """
        B, S = input_ids.shape
        ...
        # Drop every position the caller will not read BEFORE the lm_head, so the
        # [B, S, vocab] tensor is never built. Must come after the final norm —
        # RMSNorm is per-position, so slicing here changes nothing numerically.
        if logits_at is not None:
            h = jnp.take_along_axis(
                h, logits_at.astype(jnp.int32)[:, None, None], axis=1)   # [B, 1, H]
...
def prefill_with_kv_cache(
    ...
    # Logits at the last REAL token of each row. Pushed into the model via
    # logits_at so the lm_head runs on one position per sequence instead of S —
    # same values, without materializing [B, S, 262144] only to discard all but
    # one row of it.
...
def chunked_prefill_with_kv_cache(
    ...
    # donate_argnums=(1, 2): caches and the validity mask are threaded through and
    # reassigned every chunk, exactly as they are in make_cached_decode_step, so
    # donating lets XLA update them in place instead of allocating a fresh copy of
    # the whole KV cache per chunk.
    #
    # This is not just an allocation saving — without it XLA's schedule collapses at
    # depth. Measured on the 31B (60 layers, chunk 256, window_kv=False, v6e-1,
    # 2026-07-31), compile-time temp for ONE step:
    #
    #   total_len   donate=False   donate=True
    #        1024       8.785 GB      8.520 GB
    #        2048       8.027 GB      8.793 GB
    #        4096   OOM (34.70 GB)    8.013 GB     <- 4.3x, and the difference
    #                                                 between working and not
    #
    # Below 4096 it is scheduling noise; at 4096 it is the whole ballgame. The
    # chunked path was previously believed unusable on this model because of this.
    # See benchmarks/runs/2026-07-31-gemma4-31b-v6e1/REPORT.md, Addendum 6.
    #
    # Safe here because nothing outside this loop holds a reference to `caches` or
    # `valid` across iterations — each is immediately rebound from the return value.
